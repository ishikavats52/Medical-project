...
const list_patient= async (req,res)=>{
   const {reports,location,email,symptoms,age,time,city}=req.body;
...
// console.log(doctor_detail)
    ...
    // if (!finddata ) {
        ...
        const userreport= await listed_user.create({username:userexist.username,phone:userexist.phone,email,doctor_detail:doctor_detail,location,symptoms,city,age,time,reports:reports});

       
        // res.status(200).json({userreport});
    // }
    if (!doctorlist) {
      const createdoc= await doctor_model.create({ doctor_name:doctor_name,email:doc_email,patient:{userexist,symptoms,age,time,city}})
      // console.log("usercreted1",createdoc)
      return ;
      
    }
      // const updated= await listed_user.findOneAndUpdate({email},  { $push:{reports:reports, doctor_detail: doctor_detail}, $set: {
      //   location: location,
      //   symptoms: symptoms,
      //   age: age,
      //   time: time, 
  ...
  const updatedoc= await doctor_model.findOneAndUpdate({email:doc_email},{$push:{patient:{userexist,symptoms,age,time,city}}},{new:true})
